...
матожидание ~ 1 начальный момент
Для любой случайной величины X центральный момент 1-го поряд-
ка равен нулю:

as[X]=integral(-inf -> inf)(x^s * f(x) * dx)
...
Дисперсия св ~ M[(X с точкой)^2], X с точкой - это X - Mx; формулы (для общего развития):
...
![Alt text](/proizv.png?raw=true "proizv")

### параметры биномиального распределения, 

Дискретная случайная величина X имеет биноминальное распределение, если ее возможные значения 0, 1, 2,..., m,..., n имеют соответствующие вероятности:

![Alt text](/4.15.png?raw=true "4.15")

биномиальное распределение зависит от 2 параметров *n* и *p*

![Alt text](/4.17.png?raw=true "4.17")

### пуассоновского распределения, 

![Alt text](/4.18.png?raw=true "4.18")

Распределение Пуассона зависит лишь от одного параметра a, который является одновременно и математическим ожиданием, и дисперсией
случайной величины X.

### геометрического распределения

Дискретная случайная величина X имеет геометрическое распределение, если ее возможные значения 0, 1, 2,..., m,...(бесконечное, но счетное множество) имеют вероятности:

Pm = q^m * p, для m =0,1,2 где 0<p<1

Вероятности Pm для последовательных значений m образуют геометрическую прогрессию с первым членом p и знаменателем q

На практике геометрическое распределение появляется при независимых испытаниях с целью получения положительного результата – наступления события A, вероятность появления которого равна p Случайная величина X – число неудачных попыток – имеет геометрическое распределение

D[X] = q/(p^2) SrKv = D^0.5


### гипергеометрического распределений, 

Дискретная случайная величина X имеет гипергеометрическое распределение с параметрами a, b, n, если ее возможные значения 0, 1, 2,..., m
имеют вероятности:

![Alt text](/hyperGeo.png?raw=true "hyperGeo")

Модель этого распределения такова: имеется урна, в которой a белых и b черных шаров; из урны вынимается n шаров. Случайная величина X – это число белых шаров среди вынутых. Важнейшие числовые характеристики случайной величины X ,имеющей гипергеометрическое распределение (без вывода),

mx=n a /(a+b)

![Alt text](/4.23.png?raw=true "4.23")

при a->inf, b->inf шаров так много, что вероятность достать из урны один белый шар p=a/(a+b)не изменяется при вынимании из урны n шаров любого цвета. В этом случае гипергеометрическое распределение можно аппроксимировать биноминальным распределением с параметрами n и p.

mx = np, Dx = npq

где q = b/(a+b) - вероятность достать черный шар

### поток событий 

Рассматривается следующая задача: на временной оси 0t случайным образом возникают точки – моменты появления каких-то однородных событий, например телефонных вызовов или обращений к серверу. Последовательность таких моментов (вызовов) называют потоком событий.
поток может обладть свойствами Стационарности, Ординарности, Отсутствия последействия

### условия, необходимые для существования пуассоновского потока. 

Потоки, обладающие этими тремя свойствами (смотри ниже), называются простейшими потоками событий. Простейший поток тесно связан с распределением Пуассона и поэтому часто называется стационарным пуассоновским потоком. 

!!!Стационарность потока не является обязательным для того, чтобы число событий, попадающих на участок , имело распределение Пуассона.

Поэтому важно чтоб имело ординарность и отсутсвие последействия.

### Стационарность

![Alt text](/1sv.png?raw=true "1sv")

### Ординарность

![Alt text](/2sv.png?raw=true "2sv")

### Отсутствие последействия

![Alt text](/3sv.png?raw=true "3sv")

## **Лекция 8**

### функции плотности и числовые характеристики равномерно распределенной случайной величины,  

Непрерывная случайная величина X имеет равномерное распреде ление на участке от a до b, если ее плотность распределения f(x) на этом участке постоянна:

![Alt text](/4.24.png?raw=true "4.24")
...
mX = (a+b)/2 = integral(-inf inf)(x * f(x) dx) = integral(a->b) (x * 1/(b-a) * dx)

Dx = integral (-inf inf)(x - mx)^2f(x)dx = ((b-a)^2)/12
...
mu4 = integral(-inf inf)(x-mx)^4 f(x)dx = integral(a->b) (((x - (a+b)/2)^4)/b-a dx) = (b-a^4)/80 

excess = mu(4)/SrednKv^4 -3
...
λ>0 - единственный параметр распределения

F(x) = 1 - e^(-λx) 

mX = integral(0 inf)(x*f(x)*dx)=1/λ

//e^-x -> 0 быстрее чем любая степень x 

Dx = 1/(λ^2)

SrednKv = 1/λ ~ mx

mu3 = integral(0 inf)((x-1/λ)^3 * λ * e^(-λx) * dx) = 2/(λ^3)

Sx(ASSymetry) = mu3/SrednKv^3 ***= 2***


### функции плотности и числовые характеристики  нормальнораспределенной  случайной величины 

Случайная величина X распределена по нормальному (гауссовому) закону с параметрами m и ,SrKv если ее плотность распределения имеет вид

![Alt text](/4.29.png?raw=true "4.29")

Кривая нормального распределения имеет симметричный холмообразный вид. Максимальное значение кривой, равное 1/sqrt(2*PI*SrKv), достигается при x=m, т. е. мода
MX=m.

матожидание

![Alt text](/4.29.5.png?raw=true "4.29.5")

![Alt text](/4.30.png?raw=true "4.30")
...
![Alt text](/normdisp.png?raw=true "normdisp")
...
размерности SrKv и m совпадают с размерностью св X. Положение кривой распределения и ее форма полностью определяются параметрами SrKv и m

![Alt text](/4.33.png?raw=true "4.33")

![Alt text](/exNorm.png?raw=true "exNorm")

![Alt text](/4.36.png?raw=true "4.36")

F(x) = 1/2 + Ф((x-m)/SrKv) Ф - гаусса

Модель нормального распределения. Складывается большое количество независимых случайных величин X1 X2 ... Xn
X = sum(Xi)

чем больше N - тем ближе к нормальному и неважно че за законы у Xi
...
![Alt text](/841.png?raw=true "841")

![Alt text](/842.png?raw=true "842")
